"""SPLITTER_CONTRACT_VIOLATION probe — Zekan's own grouped splitter self-check.

Internal integrity test: verifies that Zekan's grouped splitter never placed the
same entity in both train and test within any single fold.  If this probe fires,
the bug is in Zekan's splitter code, not in the user's data or split choice.
...
Wording contract
----------------
All narrative frames Zekan as the source of any violation — "Zekan's grouped
...
_WHY = (
    "Zekan's grouped splitter guarantees that no entity appears in both train "
...
def probe_splitter_contract_violation(
    ...
    """Internal self-check: did Zekan's own grouped splitter honour entity grouping?
    ...
    if not affected_folds:
        return IssueRecord(
            ...
            status="pass",
            what=(
                "Zekan's grouped splitter correctly held out entire entities: "
    ...
    return IssueRecord(
        ...
        what=(
            f"Zekan's grouped splitter placed the same entity in both train and "
        ...
        next_fix=(
            "File a bug against zekan.severity.splitters — the grouped splitter "
