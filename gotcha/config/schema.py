"""Top-level gotcha.yml schema: contract + model spec + split policy."""
...
class SplitPolicy(BaseModel):
    """Controls how gotcha creates train/test folds for both protocols.
...
class GotchaConfig(BaseModel):
    """Top-level configuration for a gotcha audit run."""
...
def load_config(path: str | Path) -> GotchaConfig:
    """Load a GotchaConfig from a gotcha.yml file."""
    with open(path) as fh:
        data = yaml.safe_load(fh)
    return GotchaConfig(**data)
